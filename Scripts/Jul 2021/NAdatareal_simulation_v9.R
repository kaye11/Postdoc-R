...
NAdata_merge <- read.csv("D:/Users/karengb/Exported Tables/NAdata_merge_apr2021.csv")
...
#save data
write.table (density_merged, "Exported Tables/density_merged_NAdata_v9.csv", sep=",", col.names=T, row.names=F)
...
hist_abundance_ent.png <- ggplot (data=density_merged, aes(x=x,  y= y, color=depthf, fill=depthf)) + 
  ...
  theme(legend.title = element_blank(), legend.key.width = unit (1, "cm"), strip.text = element_text(face="italic")) 
...
abundance.png <- ggplot(data = world)+ geom_sf(color="darkgray", fill="gainsboro") + coord_sf(xlim = c(-70, 10), ylim = c(30, 75) , expand = FALSE)+   geom_point(data=NAdata_merge, aes(x = lon, y = lat, color = log10(abundance)))+ 
  ...
  theme(legend.position = "right", legend.direction = "vertical", legend.key.height = unit (4, "line"),
        ...
        legend.box.margin=margin(10,10,10,10),  strip.text = element_text(face="italic")) + 
...
BM <- read.csv("D:/Users/karengb/CSV Files/beta_master9_BM.csv", sep=";")
DS <- read.csv("D:/Users/karengb/CSV Files/beta_master9_DS.csv", sep=";")
...
NAdata.betas_30m$beta_turb <- (0.42*pi*((NAdata.betas_30m$disrate_30m/(v))^0.5)*((NAdata.betas_30m$rad+Rehv)^3))*86400*10^6 
...
resize.win(17,6)
density30m_plot_Ehux.png
density30m_plot_Ehux <- density30m_plot_Ehux.png
ggsave(filename = "density30m_plot_Ehux", width = 17, height = 6, dpi = 600, device="pdf", units="in")
...
resize.win(13, 6)
density30m_plot_lith.png
density30m_plot_lith <- density30m_plot_lith.png
ggsave(filename = "density30m_plot_lith", width = 13, height = 6, dpi = 600, device="pdf", units="in")
...
NAdata.betas_1m$beta_turb <- (0.42*pi*((NAdata.betas_1m$disrate_1m/(v))^0.5)*((NAdata.betas_1m$rad+Rehv)^3))*86400*10^6 
...
resize.win(17,6)
density1m_plot_Ehux.png
density1m_plot_Ehux <- density1m_plot_Ehux.png
ggsave(filename = "density1m_plot_Ehux", width = 17, height = 6, dpi = 600, device="pdf", units="in")
...
resize.win(13,6)
density1m_plot_lith.png
density1m_plot_lith <- density1m_plot_lith.png
ggsave(filename = "density1m_plot_lith", width = 13, height = 6, dpi = 600, device="pdf", units="in")
...
infcomb30m_plot.png <- ggplot (data=infcomb_30m, aes(x=log10(percent), y=y, fill=depthf, color=depthf)) +
  ...
  scale_color_manual (values= c("#998ec3", "#f1a340")) + scale_fill_manual (values= c("#998ec3", "#f1a340")) + 
  scale_x_continuous(breaks = c(2, 1, 0, -1, -2, -3, -4),label = c(100, 10, 1, 0.1, 0.01, 0.001, 0.0001)) +
...
##adscomb lith map
adscomblith_30m <- melted_NAdata.betas_comb_30m %>% filter (group=="Li") %>% filter (parameter=="adscomb") %>%
  mutate (mldepth="30 m")

adscomblith_1m <- melted_NAdata.betas_comb_1m %>% filter (group=="Li") %>% filter (parameter=="adscomb") %>%
  mutate (mldepth="1 m")

adscomb_combmap <-rbind(adscomblith_1m, adscomblith_30m)

adscomb_map.png <-ggplot(data = world)+ geom_sf(color="gray", fill="gainsboro") + 
  coord_sf(xlim = c(-70, 10), ylim = c(30, 75) , expand = FALSE)+
  geom_point(data=adscomb_combmap,
  ...
  theme(legend.position = "right", legend.direction = "vertical", legend.key.height = unit (4, "line"),
        ...
        panel.spacing = unit(3, "lines"), axis.title = element_blank(), legend.margin=margin(0,0,0,0),
        legend.box.margin=margin(10,10,10,10)) + 
  labs (color= expression("% adsorbed"~d^-1), x="", y="") +
  guides(color = guide_colorbar(title.position = "right")) + facet_grid(~mldepth)

ggsave(filename = "adscomb_map.png", width = 12, height = 6, dpi = 600, device="png", units="in")

##prob density
adscomblith_30m_dens <- density_30m %>% filter (entity=="coccolith") %>% filter (parameter=="adscomb") %>%
  mutate (mldepth="30 m")

adscomblith_1m_dens <- density_1m %>% filter (entity=="coccolith") %>% filter (parameter=="adscomb") %>%
  mutate (mldepth="1 m")

adscomb_combdens <-rbind(adscomblith_1m_dens, adscomblith_30m_dens)

adscomb_lith_dens.png <- ggplot (data=adscomb_combdens, 
        aes(x=log10((10^x)*100), y=y, fill=depthf, color=depthf)) +
  geom_area(position = "identity", alpha=0.3) +   geom_line(size=1) + geom_vline(xintercept = 2, linetype="dashed") +
  scale_color_manual (values= c("#998ec3", "#f1a340")) + scale_fill_manual (values= c("#998ec3", "#f1a340")) + 
  scale_x_continuous(breaks = c(2, 1, 0, -1, -2, -3, -4),label = c(100, 10, 1, 0.1, 0.01, 0.001, 0.0001), limits = c(-4, 4)) +
  labs (x=expression("% population with\nadsorbed EhVs"~d^-1), y="probability density") +   coord_flip() + 
  theme_Publication2() +  
  theme(legend.title = element_blank(), legend.key.width = unit (2, "line"), panel.spacing = unit(3, "lines"), 
        legend.position = "bottom", axis.title.y = element_text(vjust=0.001)) + facet_grid(~mldepth)

ggsave(filename = "adscomb_lith_dens.png", width = 11, height = 6, dpi = 600, device="png", units="in")
resize.win (11, 6)
...
NA_infection_coords_1m <-ggplot(data = world)+ geom_sf(color="gray", fill="gainsboro") + 
  ...
  theme(legend.position = "right", legend.direction = "vertical", legend.key.height = unit (4, "line"),
        ...
        panel.spacing = unit(3, "lines"), axis.title = element_blank(), legend.margin=margin(0,0,0,0),
        legend.box.margin=margin(10,10,10,10)) + 
  labs (color= expression("% population infected"~d^-1), x="", y="") +
  guides(color = guide_colorbar(title.position = "right")) +
...
naviceslice_coords_origcolor_percent1m.png <- ggplot(data = world)+ geom_sf(color="gray", fill="gainsboro") + 
  ...
  theme(legend.position = "right", legend.direction = "vertical", legend.key.height = unit (4, "line"),
        ...
        panel.spacing = unit(3, "lines"), axis.title = element_blank(), legend.margin=margin(0,0,0,0),
        legend.box.margin=margin(10,10,10,10)) + 
  labs (color= expression("% populationinfected"~d^-1), x="", y="") +
  guides(color = guide_colorbar(title.position = "right")) +
  geom_point(data=coords, aes(x=lon, y=lat), color="black", size=4)
