...
##pending
#1. histograms
#2. color of the maps, everything has already been run
...
ggplot (data=NAdata_merge, aes(x=log10(abundance), color=depthf, fill=depthf)) +
  geom_histogram(binwidth = 0.1, aes(y=..density..), alpha=0.75) +
  geom_density(alpha=.2, position="stack", color="black", size=0.5) + 
  scale_x_continuous(n.breaks = 6) +
  theme_Publication2() + 
  labs (x = expression(log[10]~"concentration"~mL^-1)) +  theme(legend.title = element_blank(), legend.key.width = unit (1, "cm")) +   facet_grid(~entity)
...
#histogram
ggplot (data=NAdata_merge, aes(x=log10(abundance), color=depthf, fill=depthf)) +
  geom_histogram(binwidth = 0.25, color="black", size=1) +
