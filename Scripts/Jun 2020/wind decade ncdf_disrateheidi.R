...
windmax <- c("max_wind_speed") #just choose wind max because you would need to recalculate disrate anyway
...
#write into table
write.table (wind, "D:/Users/karengb/Exported Tables/winddisrate_heidi.csv", sep=",", col.names=T, row.names=F)
...
ggplot(data = world)+ geom_sf(color="white", fill="white") + coord_sf(xlim = c(-59, 10), ylim = c(30, 70) , expand = FALSE)+   geom_point(data=wind, aes(x = lon, y = lat, color = max_wind_speed))+
  ...
  #scale_color_continuous(type="viridis", na.value="white") +
 # scale_color_viridis_c (na.value = "white") +
  #scale_color_discrete() +
  scale_color_carto_c(palette="BrwnYl", direction=1) +
