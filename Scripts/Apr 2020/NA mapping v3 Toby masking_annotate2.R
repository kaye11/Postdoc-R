...
##used to merge with Ali's data
write.table (NAdata, "Postdoc-R/Exported Tables/NAdata_mergewithAli.csv", sep=",", col.names=T, row.names=F)
...
ggplot(data = world)+ geom_sf(color="white", fill="white") + coord_sf(xlim = c(-34, -30), ylim = c(53, 62) , expand = FALSE)+  geom_point(data=navice %>% filter (entity=="Li") %>% filter (!(step=="infcomb")), 
             aes(x = lon, y = lat, color = log10(value)))+
  scale_color_viridis(breaks = c(seq(0, 6, 2))) + expand_limits(colour = c(seq(0, 6, 2))) +
  theme_Publication2() + theme(legend.position = "right", legend.direction = "vertical", legend.key.height = unit (5, "line"), legend.key.width = unit (2, "line"),  legend.title=element_blank(), panel.spacing = unit(3, "lines"), axis.title = element_blank(), plot.margin = margin(0, -20, 0, -20, "cm")) + 
  facet_grid(watcon~step2)
