...
wind_path <- paste0 ("D:/Postdoc/theoretical/NA simulation data/Modelling virus encounters/wind_dissipation_NA_2010_2018.nc")
...
length(disrate.vec)
disrate <- data.frame(cbind(lonlat, disrate.vec))
names(disrate) <- c("lon", "lat", paste(disratemax))
head(na.omit(disrate), 5)
#converting disrate, changed from 15 to 18
den18 <- 1025 #m2/kg
disrate$disrate_1m <-  (10^(2.688*(log10(wind$max_wind_speed)) - 1.322* (log10(1)) -4.812))/den18 #dividing by den18 will convert W/m3 to m2/s3, the other whole equation was from the paper by Mackenzie & Leggett. Change the depth log10(30 or 1)
disrate$disrate_30m <-  (10^(2.688*(log10(wind$max_wind_speed)) - 1.322* (log10(30)) -4.812))/den18 #dividing by den18 will convert W/m3 to m2/s3, the other whole equation was from the paper by Mackenzie & Leggett. Change the depth log10(30 or 1)

#merge
winddis <- inner_join(wind, disrate) #checked merging via head, tail and df [c(row#n, row#n)]

#write into table
write.table (wind, "Postdoc-R/Exported Tables/wind.csv", sep=",", col.names=T, row.names=F)
write.table (disrate, "Postdoc-R/Exported Tables/disrate.csv", sep=",", col.names=T, row.names=F)
write.table (winddis, "Postdoc-R/Exported Tables/winddisrate.csv", sep=",", col.names=T, row.names=F)
...
ggplot(data = world)+ geom_sf(color="white", fill="white") + coord_sf(xlim = c(-59, 10), ylim = c(30, 70) , expand = FALSE)+ 
  geom_point(data=wind, aes(x = lon, y = lat, color = max_wind_speed))+
...
#disrate_upper1m
ggplot(data = world)+ geom_sf(color="white", fill="white") + coord_sf(xlim = c(-59, 10), ylim = c(30, 70) , expand = FALSE)+ 
  geom_point(data=winddis, aes(x = lon, y = lat, color = log10(disrate_1m)))+
...
#disrate_30m
ggplot(data = world)+ geom_sf(color="white", fill="white") + coord_sf(xlim = c(-59, 10), ylim = c(30, 70) , expand = FALSE)+ 
  geom_point(data=winddis, aes(x = lon, y = lat, color = log10(disrate_30m)))+
  scale_color_viridis_c (na.value = "white") +
