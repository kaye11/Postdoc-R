...
##wind speed
maxwind.png <- ggplot(data = world)+ geom_sf(color="white", fill="white") + coord_sf(xlim = c(-70, 10), ylim = c(30, 75) , expand = FALSE)+ 
...
#calculate viral abundance, Cc:virus is 1:10 Li:virus = 1:1
...
melted_NAdata.betas_comb_1m$step2<- factor (melted_NAdata.betas_comb_1m$step, levels= c("abundance", "enccomb", "adscomb", "infcomb"), labels = c("abundance~mL^{-1}", "encounters~d^{-1}~mL^{-1}", "adsorptions~d^{-1}~mL^{-1}", "lytic~infections~\n~d^{-1}~mL^{-1}"))
...
melted_NAdata.betas_comb_30m$step2<- factor (melted_NAdata.betas_comb_30m$step, levels= c("abundance", "enccomb", "adscomb", "infcomb"), labels = c("abundance~mL^{-1}", "encounters~d^{-1}~mL^{-1}", "adsorptions~d^{-1}~mL^{-1}", "lytic~infections~\n~d^{-1}~mL^{-1}"))
...
ggplot(data=NAdata_merge, aes(x=max_wind_speed, fill=depthf)) +  geom_density(position = "identity", alpha=0.5)
