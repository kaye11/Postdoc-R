...
melted_NAabundancedata <- reshape2::melt (NA_abundancedata, id.vars = c("lat", "lon", "waterdepth_m", "depthf", "depthf100"), value.name = "abundance", variable.name = "entity")

melted_NAabundancedata <- melted_NAabundancedata %>% filter (!(abundance < 10)) 
...
ggplot (data=melted_NAabundancedata, aes(x=log10(abundance), color=depthf, fill=depthf)) + 
  ...
  geom_density(data=melted_NAabundancedata, aes(x=log10(abundance), color=depthf100, fill=depthf100), 
                                                position="stack", alpha=0.2) +
  theme_Publication2() + labs (x = expression("log abundance"~mL^-1)) +  theme(legend.title = element_blank()) + 
  facet_grid(~entity)

#100m
ggplot (data=melted_NAabundancedata, aes(x=log10(abundance), color=depthf100, fill=depthf100)) + 
  geom_density(position = "stack", alpha=0.2) +
  theme_Publication2() + labs (x = expression("log abundance"~mL^-1)) +  theme(legend.title = element_blank()) + 
  facet_grid(~entity)
...
levelplot(log10(abundance) ~ lon * lat | entity, data=melted_NAabundancedata %>%  
          ...
          preif gtty=TRUE, col.regions = viridis(256, begin=0, end=1), cuts=10,
...
ggplot(data = world)+ geom_sf()+ coord_sf(xlim = c(-70, 10), ylim = c(30, 75) , expand = FALSE)+ 
  geom_point(data=melted_NAabundancedata %>%  filter(entitycode %in% c("Ncells_max", "Nliths_max")), 
             aes(x = lon, y = lat, color = log10(abundance)))+ scale_color_viridis(breaks = c(seq(-6, 10, 2))) + 
...
resize.win(27,24)
levelplot(log10(value) ~ lon * lat | step2*watcon, data=melted_NAdata.betas_comb %>%
...
write.table (all.sum, "Postdoc-R/Exported Tables/NAsimulation_calmstormy_sum_v2.csv", sep=",", col.names=T, row.names=F)
write.table (all.sum.withdepth, "Postdoc-R/Exported Tables/NAsimulation_calmstormy_sum__withdepth_v2.csv", sep=",", col.names=T, row.names=F)
