...
source("inspack.R")
library(readxl)
navice <- read_excel("D:/Postdoc/theoretical/NA-VICE/NAVICE_long_wrangledinexcel_avebetaDS.xlsx") #averaged Cc beta_DS was used
...
#edit in excel
write.table(melted_navice, "Postdoc-R/Exported Tables/melted_navice_30m_avgbetads.csv", sep=";", col.names=T, row.names=F)
...
probs <- as.data.frame(list (entitycode = as.factor(c("Nc", "Cc", "Li", "Nc", "Cc", "Li")), virus = c("high","high", "high", "low", "low", "low"), ads= c(0.0488, 0.0169, 0.0293, 0.0488, 0.0169, 0.0293),  inf = c(0.3, 0.3, NA, 0.06, 0.06, NA)))
...
melted_EI3$variable <- factor (melted_EI3$variable,levels= c("abundance", "enccomb", "adscomb", "infcomb"), 
                               labels = c("abundance mL-1", "encounters day-1 mL-1", "with adsorbed EhVs\n day-1 mL-1", "lytic infections\n day-1 mL-1"))


write.table(navice_EI, "Postdoc-R/Exported Tables/navice_EI_upper30m_avgvbetaads.csv", sep=";", col.names=T, row.names=F)
...
resize.win(9.5,4)
ggplot(melted_EI3, aes(x=entitycode, y=log10(value), color=entitycode))  + geom_boxplot() + geom_point (position=position_jitterdodge()) + facet_grid(~variable) + theme_Publication() + theme (legend.title = element_blank(), axis.title.x = element_blank(), legend.position = "none") + labs (y="log10") + scale_color_manual (values=c("#e41a1c", "#377eb8", "#4daf4a")) +   geom_hline(yintercept = log10(1), linetype="dashed") 
