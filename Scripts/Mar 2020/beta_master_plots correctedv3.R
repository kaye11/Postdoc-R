...
#encounters
calmstormy$perencounters <- (calmstormy$encounters_propEhV/calmstormy$propEhV)*100
calmstormy$peradsorbed <- (calmstormy$adstot_prop/calmstormy$propEhV)*100
calmstormy$perinf <- (calmstormy$sucinf_prop/calmstormy$propEhV)*100

ggplot(calmstormy %>% filter (group2=="Nc"), aes(x = "", y = perencounters)) +
  geom_bar(width = 1, stat = "identity", color = "white") +
  coord_polar("y", start = 0)+
  theme_void() + facet_grid(virus~condition)

datapie <- calmstormy %>% select (c(prop))
