...
NAdata_merge$entity <- factor (NAdata_merge$entity,labels= c("calcified", "coccolith"))
...
#probabilities
probs <- as.data.frame(list (entity = as.factor (rep(c("calcified","coccolith"), 2)), virus = rep(c("high", "low"), 1, each=8), propvir= rep(c(0.33, 0.67), 1, each=8), ads = rep(c(0.0243, 0.0302), 4), inf = rep(c(0.3, NA,0.3, NA, 0.3, NA, 0.3, NA, 0.06, NA, 0.06, NA, 0.06, NA, 0.06, NA), 1, each=1)))
...
all_1m.png<- ggplot(data = world)+ geom_sf(color="gray", fill="gainsboro") + 
  ...
  geom_point(data= melted_NAdata.betas_comb_1m %>% filter (! ((entity=="coccolith") & (step=="infcomb"))), 
...
Li.abundance <- ggplot (data=melted_NAdata.betas_comb_1m %>% filter (entity=="coccolith") %>% filter (step=="abundance"), 
                        aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Li.abundance <- ggplot_build(Li.abundance)$data[[1]] %>% select (x, y)
Li.abundance$entity <- "coccolith"
Li.abundance$step <- "abundance"

Li.enccomb <- ggplot (data=melted_NAdata.betas_comb_1m %>% filter (entity=="coccolith") %>% filter (step=="enccomb"), 
                      aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Li.enccomb <- ggplot_build(Li.enccomb)$data[[1]] %>% select (x, y)
Li.enccomb$entity <- "coccolith"
Li.enccomb$step <- "enccomb"

Li.adscomb <- ggplot (data=melted_NAdata.betas_comb_1m %>% filter (entity=="coccolith") %>% filter (step=="adscomb"), 
                      aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Li.adscomb <- ggplot_build(Li.adscomb)$data[[1]] %>% select (x, y)
Li.adscomb$entity <- "coccolith"
...
all_30m.png<- ggplot(data = world)+ geom_sf(color="gray", fill="gainsboro") + 
  coord_sf(xlim = c(-70, 10), ylim = c(30, 75) , expand = FALSE)+ 
  geom_point(data= melted_NAdata.betas_comb_30m %>% filter (! ((entity=="coccolith") & (step=="infcomb"))), 
             aes(x = lon, y = lat, color = log10(value)))+
  scale_colour_gradientn(colours = BlRed(100), breaks = c(seq (-5,8, 2)), limits=c(-5, 8)) +
  theme_Publication2() + theme(legend.position = "right", legend.direction = "vertical", legend.key.height = unit (4, "line"), legend.key.width = unit (1.25, "line"),  legend.title=element_blank(), panel.spacing = unit(3, "lines"), axis.title = element_blank(),  legend.margin=margin(0,0,0,0),
                               legend.box.margin=margin(10,10,10,10)) + facet_grid(entity~step, labeller = labeller(step = as_labeller(variable_labs, label_parsed))) + labs (x="", y="") 
...
Li.abundance.30m <- ggplot (data=melted_NAdata.betas_comb_30m %>% filter (entity=="coccolith") %>% filter (step=="abundance"), 
                            aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Li.abundance.30m <- ggplot_build(Li.abundance.30m)$data[[1]] %>% select (x, y)
Li.abundance.30m$entity <- "coccolith"
Li.abundance.30m$step <- "abundance"

Li.enccomb.30m <- ggplot (data=melted_NAdata.betas_comb_30m %>% filter (entity=="coccolith") %>% filter (step=="enccomb"), 
                          aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Li.enccomb.30m <- ggplot_build(Li.enccomb.30m)$data[[1]] %>% select (x, y)
Li.enccomb.30m$entity <- "coccolith"
Li.enccomb.30m$step <- "enccomb"

Li.adscomb.30m <- ggplot (data=melted_NAdata.betas_comb_30m %>% filter (entity=="coccolith") %>% filter (step=="adscomb"), 
                          aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Li.adscomb.30m <- ggplot_build(Li.adscomb.30m)$data[[1]] %>% select (x, y)
Li.adscomb.30m$entity <- "coccolith"
