...
#save data
write.table (density_merged, "Exported Tables/density_merged_NAdata_v8.csv", sep=",", col.names=T, row.names=F)
...
density30m_plot_Ehux.png <- ggplot (data=density_30m %>% filter (entity=="E. huxleyi"), 
  ...
  theme(legend.title = element_blank(), legend.key.width = unit (2, "line"), panel.spacing = unit(3, "lines"), 
        legend.position = "bottom", panel.grid.major = element_line(size = 0.5, linetype = 'solid', colour = "#D3D3D3")) + 
  facet_grid(~parameter, labeller = labeller(parameter = as_labeller(variable_labs, label_parsed)))
...
adscomb_lith_dens.png <- ggplot (data=adscomb_combdens, 
  ...
  theme(legend.title = element_blank(), legend.key.width = unit (2, "line"), panel.spacing = unit(3, "lines"), 
        ...
        panel.grid.major = element_line(size = 0.5, linetype = 'solid', colour = "#D3D3D3")) + facet_grid(~mldepth)
