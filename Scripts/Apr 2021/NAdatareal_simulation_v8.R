...
density30m_plot_lith.png <- ggplot (data=density_30m %>% filter (entity=="coccolith"), 
  ...
  labs (x=expression(log[10]), y="probability density") +   coord_flip() + theme_Publication2() +  
  theme(legend.title = element_blank(), legend.key.width = unit (2, "line"), panel.spacing = unit(3, "lines"), 
        legend.position = "bottom") + xlim(-6, 6) +
  facet_grid(~parameter, labeller = labeller(parameter = as_labeller(variable_labs, label_parsed)))
...
adscomblith_30m.png <- ggplot (data=density_30m %>% filter (entity=="coccolith") %>% filter (parameter=="adscomb"), 
...
ggsave(filename = "adscomblith_30m.png", width = 6, height = 6, dpi = 600, device="png", units="in")


adscomblith_1m.png <-ggplot (data=density_1m %>% filter (entity=="coccolith") %>% filter (parameter=="adscomb"), 
        aes(x=log10((10^x)*100), y=y, fill=depthf, color=depthf)) +
  geom_area(position = "identity", alpha=0.3) +   geom_line(size=1) + geom_vline(xintercept = 2, linetype="dashed") +
  scale_color_manual (values= c("#998ec3", "#f1a340")) + scale_fill_manual (values= c("#998ec3", "#f1a340")) + 
  scale_x_continuous(breaks = c(2, 1, 0, -1, -2, -3, -4),label = c(100, 10, 1, 0.1, 0.01, 0.001, 0.0001), limits=c(-4,4)) +
...
ggsave(filename = "adscomblith_1m.png", width = 6, height = 6, dpi = 600, device="png", units="in")
