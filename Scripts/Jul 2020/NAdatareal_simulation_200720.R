...
maxwind.png <- ggplot(data = world)+ geom_sf(color="darkgray", fill="gainsboro") + coord_sf(xlim = c(-70, 10), ylim = c(30, 75) , expand = FALSE)+ 
  ...
  theme_Publication2() + theme(legend.position = "right", legend.direction = "vertical", legend.key.height = unit (4, "line"), legend.key.width = unit (1.25, "line"),  legend.title=element_blank(), panel.spacing = unit(3, "lines"), axis.title = element_blank(), legend.margin=margin(0,0,0,0),
                               legend.box.margin=margin(10,10,10,10)) + labs (title= expression("Wind speed("~m~s^-1~")"))  + labs (x="", y="") 
...
all_1m.png<- ggplot(data = world)+ geom_sf(color="gray", fill="gainsboro") + 
  ...
  geom_point(data= melted_NAdata.betas_comb_1m %>% filter (! ((entity=="Li") & (step=="infcomb"))), 
...
Cc.abundance <- ggplot (data=melted_NAdata.betas_comb_1m %>% filter (entity=="Cc") %>% filter (step=="abundance"), 
                        aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Cc.abundance <- ggplot_build(Cc.abundance)$data[[1]] %>% select (x, y)
Cc.abundance$entity <- "Cc"
Cc.abundance$step <- "abundance"

Cc.enccomb <- ggplot (data=melted_NAdata.betas_comb_1m %>% filter (entity=="Cc") %>% filter (step=="enccomb"), 
                      aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Cc.enccomb <- ggplot_build(Cc.enccomb)$data[[1]] %>% select (x, y)
Cc.enccomb$entity <- "Cc"
Cc.enccomb$step <- "enccomb"

Cc.adscomb <- ggplot (data=melted_NAdata.betas_comb_1m %>% filter (entity=="Cc") %>% filter (step=="adscomb"), 
                      aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Cc.adscomb <- ggplot_build(Cc.adscomb)$data[[1]] %>% select (x, y)
Cc.adscomb$entity <- "Cc"
Cc.adscomb$step <- "adscomb"

Cc.infcomb <- ggplot (data=melted_NAdata.betas_comb_1m %>% filter (entity=="Cc") %>% filter (step=="infcomb"), 
                      aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Cc.infcomb <- ggplot_build(Cc.infcomb)$data[[1]] %>% select (x, y)
Cc.infcomb$entity <- "Cc"
Cc.infcomb$step <- "infcomb"

Li.abundance <- ggplot (data=melted_NAdata.betas_comb_1m %>% filter (entity=="Li") %>% filter (step=="abundance"), 
                        aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Li.abundance <- ggplot_build(Li.abundance)$data[[1]] %>% select (x, y)
Li.abundance$entity <- "Li"
Li.abundance$step <- "abundance"

Li.enccomb <- ggplot (data=melted_NAdata.betas_comb_1m %>% filter (entity=="Li") %>% filter (step=="enccomb"), 
                      aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Li.enccomb <- ggplot_build(Li.enccomb)$data[[1]] %>% select (x, y)
Li.enccomb$entity <- "Li"
Li.enccomb$step <- "enccomb"

Li.adscomb <- ggplot (data=melted_NAdata.betas_comb_1m %>% filter (entity=="Li") %>% filter (step=="adscomb"), 
                      aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Li.adscomb <- ggplot_build(Li.adscomb)$data[[1]] %>% select (x, y)
Li.adscomb$entity <- "Li"
...
all_30m.png<- ggplot(data = world)+ geom_sf(color="gray", fill="gainsboro") + 
  coord_sf(xlim = c(-70, 10), ylim = c(30, 75) , expand = FALSE)+ 
  geom_point(data= melted_NAdata.betas_comb_30m %>% filter (! ((entity=="Li") & (step=="infcomb"))), 
             aes(x = lon, y = lat, color = log10(value)))+
  scale_colour_gradientn(colours = BlRed(100), breaks = c(seq (-5,8, 2)), limits=c(-5, 8)) +
  theme_Publication2() + theme(legend.position = "right", legend.direction = "vertical", legend.key.height = unit (4, "line"), legend.key.width = unit (1.25, "line"),  legend.title=element_blank(), panel.spacing = unit(3, "lines"), axis.title = element_blank(),  legend.margin=margin(0,0,0,0),
                               legend.box.margin=margin(10,10,10,10)) + facet_grid(entity~step, labeller = labeller(step = as_labeller(variable_labs, label_parsed))) + labs (x="", y="") 

ggsave(filename = "all_30m.png", width = 18, height = 10, dpi = 600, device="png", units="in")


#make a plot like NA-VICE but density plot
resize.win(17,5)

Cc.abundance.30m <- ggplot (data=melted_NAdata.betas_comb_30m %>% filter (entity=="Cc") %>% filter (step=="abundance"), 
                            aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Cc.abundance.30m <- ggplot_build(Cc.abundance.30m)$data[[1]] %>% select (x, y)
Cc.abundance.30m$entity <- "Cc"
Cc.abundance.30m$step <- "abundance"

Cc.enccomb.30m <- ggplot (data=melted_NAdata.betas_comb_30m %>% filter (entity=="Cc") %>% filter (step=="enccomb"), 
                          aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Cc.enccomb.30m <- ggplot_build(Cc.enccomb.30m)$data[[1]] %>% select (x, y)
Cc.enccomb.30m$entity <- "Cc"
Cc.enccomb.30m$step <- "enccomb"

Cc.adscomb.30m <- ggplot (data=melted_NAdata.betas_comb_30m %>% filter (entity=="Cc") %>% filter (step=="adscomb"), 
                          aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Cc.adscomb.30m <- ggplot_build(Cc.adscomb.30m)$data[[1]] %>% select (x, y)
Cc.adscomb.30m$entity <- "Cc"
Cc.adscomb.30m$step <- "adscomb"

Cc.infcomb.30m <- ggplot (data=melted_NAdata.betas_comb_30m %>% filter (entity=="Cc") %>% filter (step=="infcomb"), 
                          aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Cc.infcomb.30m <- ggplot_build(Cc.infcomb.30m)$data[[1]] %>% select (x, y)
Cc.infcomb.30m$entity <- "Cc"
Cc.infcomb.30m$step <- "infcomb"

Li.abundance.30m <- ggplot (data=melted_NAdata.betas_comb_30m %>% filter (entity=="Li") %>% filter (step=="abundance"), 
                            aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Li.abundance.30m <- ggplot_build(Li.abundance.30m)$data[[1]] %>% select (x, y)
Li.abundance.30m$entity <- "Li"
Li.abundance.30m$step <- "abundance"

Li.enccomb.30m <- ggplot (data=melted_NAdata.betas_comb_30m %>% filter (entity=="Li") %>% filter (step=="enccomb"), 
                          aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Li.enccomb.30m <- ggplot_build(Li.enccomb.30m)$data[[1]] %>% select (x, y)
Li.enccomb.30m$entity <- "Li"
Li.enccomb.30m$step <- "enccomb"

Li.adscomb.30m <- ggplot (data=melted_NAdata.betas_comb_30m %>% filter (entity=="Li") %>% filter (step=="adscomb"), 
                          aes(x=log10(value))) +
  geom_histogram(binwidth = 0.1, aes (y = ..count../sum(..count..)), alpha=0.25, position="identity") 

Li.adscomb.30m <- ggplot_build(Li.adscomb.30m)$data[[1]] %>% select (x, y)
Li.adscomb.30m$entity <- "Li"
...
ggplot(data = world)+ geom_sf(color="gray", fill="gainsboro") + 
  coord_sf(xlim = c(-40, -20), ylim = c(50, 70 ) , expand = FALSE) +
  geom_point(data= navice %>% filter (! (entity=="Li"))%>% filter (step=="infcomb"),  
             aes(x = lon, y = lat, color = log10(value)))+
  scale_colour_gradientn(colours = BlRed(100), breaks = c(seq (-5,8, 2)), limits=c(-5, 8)) +
  theme_Publication2() + theme(legend.position = "right", legend.direction = "vertical", legend.key.height = unit (4, "line"), legend.key.width = unit (1.25, "line"),  legend.title=element_blank(), panel.spacing = unit(3, "lines"), axis.title = element_blank(),  legend.margin=margin(0,0,0,0),
...
#put coords but with color scale changed
ggplot(data = world)+ geom_sf(color="gray", fill="gainsboro") + 
  coord_sf(xlim = c(-40, -20), ylim = c(50, 70 ) , expand = FALSE) +
  geom_point(data= navice %>% filter (! (entity=="Li"))%>% filter (step=="infcomb"),  
             aes(x = lon, y = lat, color = log10(value)))+
