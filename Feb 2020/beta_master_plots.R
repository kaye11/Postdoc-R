...
probs$bioinf <- probs$ads*probs$inf #probs was wrong repeat everything after here
...
ggplot(calmstormy, aes(x=as.factor(condition), y=log10(adstot_prop), color=as.factor(group), shape=as.factor(group))) +geom_boxplot() + geom_point (position = position_jitterdodge(), size=1) + labs (y = expression("log10 total adsorbed viruses "~day^-1)) + theme_Publication2() + theme(legend.title = element_blank(), axis.title.x = element_blank()) + facet_grid(disrate~virus) + scale_color_manual (values=c("#e41a1c", "#377eb8", "#4daf4a")) +   geom_hline(yintercept = log10(1), linetype="dashed")

#plot successful infection first
ggplot(calmstormy %>% drop_na(sucinf_prop) , aes(x=as.factor(condition), y=log10(sucinf_prop), color=as.factor(group), shape=as.factor(group))) +geom_boxplot() + geom_point (position = position_jitterdodge(), size=1) + labs (y = expression("log10 total adsorbed viruses "~day^-1)) + theme_Publication2() + theme(legend.title = element_blank(), axis.title.x = element_blank()) + facet_grid(disrate~virus) +  scale_color_manual (values=c("#e41a1c", "#377eb8", "#4daf4a")) +   geom_hline(yintercept = log10(1), linetype="dashed")
