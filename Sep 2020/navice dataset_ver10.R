...
#edit in excel
write.table(melted_navice, "Postdoc-R/Exported Tables/melted_navice_30m_v10.csv", sep=";", col.names=T, row.names=F)
...
navice_EI$enc_prop <- navice_EI$beta_all*navice_EI$abundance #entitysense host
...
write.table(navice_EI, "Postdoc-R/Exported Tables/navice_EI_upper30m_v10.csv", sep=";", col.names=T, row.names=F)
...
ggplot (data=lithcellehv, aes(x=Infection.2lines, y=log10(ratvir), color=Infection)) + 
  geom_boxplot() + geom_point(size=5, position=position_jitterdodge())  +
  labs (y = "EhV:calcified E. huxleyi ratio", color="infection phase") + theme_Publication2() + scale_color_manual (values=c("#CC79A7","#fdb462","#7fc97f", "#D55E00","#662506")) + guides(colour=guide_legend(nrow=2,byrow=TRUE, title.position = "top")) + theme (legend.title=element_blank(), axis.title.x = element_blank(), legend.position = "none")
