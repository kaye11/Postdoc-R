...
write.table(calmstormy, "Postdoc-R/Exported Tables/calmstormy_master7.csv", sep=";", col.names=T, row.names=F)
...
#infected
ggplot(melt_inf.calm %>% filter(!(group %in% c("coccolith"))), aes(x = "", y = value, fill=variable)) +
  geom_bar(width = 1, stat = "identity", color = "black") +
  coord_polar("y", start = 0)+
  scale_fill_manual(values=c("#800080", "#FFFFFF")) +
  theme_void() +  theme (legend.position = "bottom", legend.text = element_text(size = 15), 
...
ads_plot$group <- factor (ads_plot$group,labels= c("naked", "calcified", "coccolith"))


resize.win(4,4)
ggplot (ads_plot, aes(x=group, y=log10(adscoef), color=group)) + geom_boxplot() + theme_Publication() + 
  theme (axis.title.x = element_blank(), legend.position = "none", axis.text.x = element_text(angle = 45, vjust = 0.6)) + labs (y= expression(log[10]~K[d]~("mL"~"day"^-1))) +
  scale_color_manual (values=c("#e41a1c", "#377eb8", "#4daf4a")) 

ggplot (ads_plot, aes(x=group, y=adsef*100, color=group)) + geom_boxplot() + theme_Publication() +  
  theme (axis.title.x = element_blank(), legend.position = "none", axis.text.x = element_text(angle = 45, vjust = 0.6)) + labs(y = bquote(delta ~ "(%)")) +
  scale_color_manual (values=c("#e41a1c", "#377eb8", "#4daf4a"))
