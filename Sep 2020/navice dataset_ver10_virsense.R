...
#edit in excel
write.table(melted_navice, "Postdoc-R/Exported Tables/melted_navice_30m_v10_virsense.csv", sep=";", col.names=T, row.names=F)
...
navice_EI_comb$perencounters <- (navice_EI_comb$enccomb/navice_EI_comb$propEhV)*100
navice_EI_comb$peradsorbed <- (navice_EI_comb$adscomb/navice_EI_comb$propEhV)*100
navice_EI_comb$perinf <- (navice_EI_comb$infcomb/navice_EI_comb$propEhV)*100
...
navice_EI_comb$perencounters_noenc <- 100- navice_EI_comb$perencounters
navice_EI_comb$perads_noads <- 100- navice_EI_comb$peradsorbed
navice_EI_comb$perinf_noinf <- 100- navice_EI_comb$perinf

#melt data to get summary of variables
melted_EI2 <- reshape2::melt(navice_EI_comb %>% select ("date2", "Infection", "entitycode", "virus", "propEhV", "abundance", "enccomb", "adscomb", "infcomb"), id.vars=c("date2", "Infection", "entitycode", "virus", "propEhV", "abundance"))

abundance <- navice_EI_comb %>% select ("date2", "Infection", "entitycode", "virus", "propEhV", "abundance")
...
write.table(navice_EI_comb, "Postdoc-R/Exported Tables/navice_EI_comb_upper30m_v10_virsense.csv", sep=";", col.names=T, row.names=F)

write.table(melted_EI3, "Postdoc-R/Exported Tables/melted_EI3_v10_virsense.csv", sep=";", col.names=T, row.names=F)
...
resize.win(6.5,6)
ggplot (data=lithcellehv, aes(x=log10(ehuxcount), y=log10(ehvcount), color=Infection)) + geom_point(size=5)  + geom_smooth(aes(group=1), color="#525252", method="loess") +   labs (y = expression(log[10]~"EhV "~mL^-1), x = expression(log[10]~"E. huxleyi "~mL^-1), color="infection phase") + theme_Publication2() +   scale_color_manual (values=c("#CC79A7","#fdb462","#7fc97f", "#D55E00","#662506")) + guides(colour=guide_legend(nrow=2,byrow=TRUE, title.position = "top")) + theme (legend.title=element_blank())
...
write.table (lithcellehv, "Postdoc-R/Exported Tables/lithcellehv_v10.csv", sep=";", col.names=T, row.names=F)
