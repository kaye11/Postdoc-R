...
calmstormy$encounters <- calmstormy$beta_all*calmstormy$prophostnum #heidi encounters
calmstormy$enc2 <- calmstormy$beta_all*calmstormy$propEhV
...
write.table(calmstormy, "Postdoc-R/Exported Tables/calmstormy_master7a_entitysense_host.csv", sep=";", col.names=T, row.names=F)
...
#calculate encounters fast slow
allhost$encounters <- allhost$beta_all*allhost$prophostnum #entity sense
...
ggplot(allhost_allsteps %>% filter (virus=="high"), aes(x=log10(prophostnum), y=log10(stormy), color="stormy"))  + 
  geom_point (size=6, position = position_dodge()) + 
  geom_point (data=allhost_allsteps %>% filter (virus=="low"), aes (x=log10(prophostnum), y=log10(mid), color="mid"), size=6, position = position_dodge()) +
  geom_point (data=allhost_allsteps %>% filter (virus=="low"), aes (x=log10(prophostnum), y=log10(calm), color="calm"), size=6, 
              position = position_dodge()) +
...
ggplot(betas_contrib, aes(x = group, y = value, fill=betasf)) +
  ...
  labs (y=expression("% contribution to"~beta)) +
