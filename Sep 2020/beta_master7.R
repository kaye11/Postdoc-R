...
require(openxlsx)
write.xlsx(BM, file = "Postdoc-R/Exported Tables/BM_master7.xlsx")
...
#PIC$beta_DS_orig <-(pi*(((PIC$rad+Rehv)*100)^2)*(abs((PIC$SinkVel-Ehv_SinkVel)/864)))*86400 #in encounters cm3/day
PIC$beta_DS <-(pi*((PIC$rad+Rehv)^2)*(abs((PIC$SinkVel-Ehv_SinkVel))))*10^6 #in encounters cm3/day
#same

PIC$beta_DS_lith <- (pi*((Reh_lith+Rehv)^2)*(abs((PIC$SinkVel_lith-Ehv_SinkVel))))*10^6 #in encounters cm3/day
...
summary_DS <- ddply(PIC, .(Strain), summarize,  PICpercellpg=mean(PICpercellpg),
                    Den_celltotal = mean (Den_celltotal),SinkVel=mean(SinkVel),
                    beta_DS=mean(beta_DS),Denlith = mean (Denlith), 
                    SinkVel_lith=mean (SinkVel_lith), 
                    beta_DS_lith=mean (beta_DS_lith))
...
write.xlsx(summary_DS, file = "Postdoc-R/Exported Tables/summary_DS_master7.xlsx")
write.xlsx(summary_DS_bygroup, file = "Postdoc-R/Exported Tables/summary_DS_bygroup_master7.xlsx")


## ------------------------------------------------------------------------
#7. make a prediction based on PIC values
#7a. for cells
#make equally spaced numbers for PIC
PIC_newdata <- as.data.frame(seq(0, 21, 0.1)) #211 values
...
#7b. calculate PIC and density for lith
PIC_newdata$Denlith <- case_when(
  PIC_newdata$group == "Nc"  ~ 1.025, #density of seawater
  PIC_newdata$group == "Cc" ~ 2.6)

ggplot(data=PIC_newdata, aes(x=group, y=Denlith, color=group)) + geom_boxplot()+geom_point(size=2)+ theme_Publication()

#7c. calculate sinking velocity of cells,liths, and viruses
...
PIC_newdata$beta_DS <-(pi*((PIC_newdata$rad+Rehv)^2)*(abs((PIC_newdata$SinkVel-Ehv_SinkVel))))*10^6 #in encounters cm3/day
#same

PIC_newdata$beta_DS_lith <- (pi*((Reh_lith+Rehv)^2)*(abs((PIC_newdata$SinkVel_lith-Ehv_SinkVel))))*10^6 #in encounters cm3/day
...
summary_DS_bygroup_pred <-ddply(PIC_newdata, .(group), summarize,  
                                PICpercellpg=mean(PICpercellpg), 
                                Den_celltotal = mean (Den_celltotal),
                                SinkVel=mean(SinkVel),beta_DS=mean(beta_DS),  
                                Denlith = mean (Denlith), SinkVel_lith=mean (SinkVel_lith), 
                                beta_DS_lith=mean (beta_DS_lith), 
                                Den_celltotal_median = median(Den_celltotal), 
                                Sinkvel_median=median (SinkVel))
...
setwd("D:/R program")
write.xlsx(summary_DS_bygroup_pred, file = "Postdoc-R/Exported Tables/summary_DS_bygroup_pred_master7.xlsx")
write.table(PIC_newdata, "Postdoc-R/Exported Tables/PIC_newdata_master7.csv", sep=";", col.names=T, row.names=F)
write.table(PIC, "Postdoc-R/Exported Tables/PIC_master7.csv", sep=";", col.names=T, row.names=F)
...
#turb$beta_turb_orig <- (4.2*pi*((turb$disrate/(v*1000^2))^0.5)*(((turb$rad+Rehv)*100)^3))*86400 this is wrong
#turb$beta_turb_orig_2 <- 4.2*pi*(((turb$disrate/v)^0.5)*86400)*(((turb$rad+Rehv)^3)*10^6) - same as 1
...
all$E_all_lowhost <- all$beta_all*all$hostlow
all$E_all_highhost <- all$beta_all*all$hosthigh
...
ggplot(data=all, aes(x=disrate,y = E_all_lowhost , color=group, fill=group)) + 
  ...
  geom_smooth(data = all, aes(y= E_all_highhost, color=group,fill=group, linetype="lab")) +
  scale_y_log10(
    breaks = scales::trans_breaks("log10", function(x) 10^x, n=2),
    labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  scale_x_log10(
