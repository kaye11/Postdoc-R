## ------------------------------------------------------------------------
#theoretical simulation with weak, moderate, and strong conditions. Beta kernels were previously computed (beta_master8.R).
source("inspack.R")

#get data frame
all <- read.csv("D:/R program/Postdoc-R/Exported Tables/beta_master9_allbetas.csv", sep=";")

#merge 3 disrates (10e-3, 10e-5, 10e-8)
weak <- all %>% filter (disrate %in% c ("1e-08")) %>% select (group, beta_BM, beta_DS, disrate, beta_turb, beta_all)
weak$watcon <- "weak"

strong <- all %>% filter (disrate %in% c ("0.001")) %>% select (group, beta_BM, beta_DS, disrate, beta_turb, beta_all)
strong$watcon <- "strong"

moderate <- all %>% filter (disrate %in% c ("1e-05")) %>% select (group, beta_BM, beta_DS, disrate, beta_turb, beta_all)
moderate$watcon <- "moderate"

weakstrong <- rbind (weak, strong, moderate)
...
#join weakstrong and probs
weakstrong <- left_join(weakstrong, probs)

#calculate propEhV
weakstrong$propEhV <- weakstrong$virnum* weakstrong$propvir

#calculate prophost
weakstrong$prophostnum <- weakstrong$hostnum* weakstrong$prophost

#calculate encounters fast slow
weakstrong$encounters <- weakstrong$beta_all*weakstrong$propEhV #encounters per entity per day

#calculate total adsorption by virus props
weakstrong$adstot <- weakstrong$encounters*weakstrong$ads

#calculate total successful infections
weakstrong$sucinf <- weakstrong$encounters*weakstrong$ads*weakstrong$inf

write.table(weakstrong, "Postdoc-R/Exported Tables/weakstrong_master9.csv", sep=";", col.names=T, row.names=F)

#make individual data table for getting the distilled plot
conc <- weakstrong[1:8, ] %>% select (group, virus, condition, prophostnum, propEhV)
conc$parameter="concentration"
conc$weak <- NA
conc$strong <- NA
conc$moderate <-conc$prophostnum

enc <- reshape(weakstrong %>% select(group, watcon, virus, condition, prophostnum, propEhV, encounters), idvar=c("group", "virus", "condition", "prophostnum", "propEhV"), timevar="watcon", direction="wide")
enc$parameter= "encounters"
ads <- reshape(weakstrong %>% select(group, watcon, virus, condition, prophostnum, propEhV, adstot), idvar=c("group", "virus", "condition", "prophostnum", "propEhV"), timevar="watcon", direction="wide")
ads$parameter= "adsorption"
inf <- reshape(weakstrong %>% select(group, watcon, virus, condition, prophostnum, propEhV, sucinf), idvar=c("group", "virus", "condition", "prophostnum", "propEhV"), timevar="watcon", direction="wide")
inf$parameter= "infections"

#renaming
enc <- setnames (enc, c("encounters.weak", "encounters.strong", "encounters.moderate"), c("weak", "strong", "moderate"))
ads <- setnames (ads, c("adstot.weak", "adstot.strong", "adstot.moderate"), c("weak", "strong", "moderate"))
inf <- setnames (inf, c("sucinf.weak", "sucinf.strong", "sucinf.moderate"), c("weak", "strong", "moderate"))
...
allsteps_comb$weak <- high$weak + low$weak
allsteps_comb$moderate <- high$moderate + low$moderate
allsteps_comb$strong <- high$strong + low$strong
...
ggplot(allsteps_comb, aes(x=group2, y=log10(weak), color="weak"))  + 
  geom_point (size=6) + #position=position_jitterdodge(dodge.width=0.2)) + 
  geom_point (data=allsteps_comb,  
              aes (x=group2, y=log10(moderate), color="moderate"), size=6) +
  geom_point (data=allsteps_comb,
              aes (x=group2, y=log10(strong), color="strong"), size=6) +
...
allsteps_comb.melt <- reshape2::melt(allsteps_comb %>% filter (!(parameter=="concentration")) %>% 
                                       select ("group", "condition", "parameter", "weak", "moderate", "strong"), 
                                     id.vars=c("group", "condition", "parameter"))

allsteps_comb.melt$percentwhole <- allsteps_comb.melt$value*100
allsteps_comb.melt$percentwholereal <- allsteps_comb.melt$value*100
...
plotpies <- ggplot(allsteps_comb.melt2 %>% filter (!(group=="Li" & parameter=="infections")), aes(x = "", y = value, fill=piefill)) +
  ...
  theme (legend.position = "none", 
         strip.text = element_text(size=15), legend.title=element_blank()) + 
  facet_grid(turb+condition~parameter+group2) + 
  geom_text(aes(y = perc, color=variable, 
                label=sprintf("%0.000f%%", round(value, digits = 5))), size=15, 
            position = position_stack(vjust = 0.5)) +
  scale_color_manual(values=c("black", "#FFFFFF")) 

ggsave(filename = "plotpies", width = 22, height = 21, dpi = 600, device="pdf", units="in")

## ------------------------------------------------------------------------
##all hosts plot
##summaries of all host entities

allhost <- weakstrong %>% filter (condition=="shelf/slope") %>% filter (group=="Cc") %>% 
...
allhost_comb$weak <- allhost_high$sucinf.weak + allhost_low$sucinf.weak
allhost_comb$moderate <- allhost_high$sucinf.moderate + allhost_low$sucinf.moderate
allhost_comb$strong <- allhost_high$sucinf.strong + allhost_low$sucinf.strong

#melt data
allhost_comb.melt <- reshape2::melt(allhost_comb %>% select (c(group, hostnum, virnum, weak, moderate, strong)), 
                                    id.vars=c("group", "hostnum", "virnum"))

resize.win(5,5)
ggplot(allhost_comb.melt, aes (x=log10(hostnum), y=log10(value*100), color=variable)) + 
  geom_line (size=2) + theme_Publication2() +    
  scale_color_manual (values=c('#2b57a7', '#ffc4b4', '#b11346'), 
                      breaks = c ("weak", "moderate", "strong")) +
...
betas_contrib <- reshape2::melt(weakstrong %>% select (group, watcon, beta_BM, beta_DS, beta_turb), id.vars=c("group", "watcon"))
...
betas_contrib$watcon <- reorder.factor (betas_contrib$watcon, new.order = c("weak", "moderate", "strong"))
...
##adsorption plot
library(readr)
ads_plot <- read_csv("Postdoc-R/CSV Files/ads_may2021.csv")
