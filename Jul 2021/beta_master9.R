...
write.table(BM, "Postdoc-R/Exported Tables/beta_master9_BM.csv", sep=";", col.names=T, row.names=F)
...
write.table(summary.DS, "Postdoc-R/Exported Tables/beta_master9_DS.csv", sep=";", col.names=T, row.names=F)
...
turb$beta_turb <- (0.42*pi*((turb$disrate/(v))^0.5)*((turb$rad+Rehv)^3))*86400*10^6 
...
write.table(all, "Postdoc-R/Exported Tables/beta_master9_allbetas.csv", sep=";", col.names=T, row.names=F)
