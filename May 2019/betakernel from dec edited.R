...
#for poster
ggplot(data=PIC, aes(x=PICpercellpg, y=E_DS_V)) +geom_point(size=5, aes(color=group2)) +
...
allbetas.sum_V <- summarySE (data=allbetas.melt %>% filter(betakernel %in% c("beta_all")) , 
                             measurevar = "E_V", 
                             groupvars=c ("group", "disrate"))

allbetas.sum_HV <- summarySE (data=allbetas.melt %>% filter(betakernel %in% c("beta_all")) , 
                              measurevar = "E_HV", 
                              groupvars=c ("group", "disrate"))
