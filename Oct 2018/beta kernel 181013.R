...
beta_BM_TK_calc <- ((4*pi)*(D_TK_host_calc+D_TK_virus)*((Reh_calc+Rehv)*100)) #Brownian motion TK formula, in cm3/s
beta_BM_TK_naked <- ((4*pi)*(D_TK_host_naked+D_TK_virus)*((Reh_naked+Rehv)*100)) #Brownian motion TK formula, in cm3/s

#3. calculate encounters (E)
#encounters are in encounters cm3/s then converted to encounters/ cm3 s
E_BM_TK_HV_calc <- beta_BM_TK_calc*(10)^4*(10)^3 #E calculated with Virus and Host (10:1 MOI)
E_BM_TK_V_calc <- beta_BM_TK_calc*(10)^4 #E calculated with Virus 
E_BM_TK_HV_naked <- beta_BM_TK_naked*(10)^4*(10)^3 #E calculated with Virus and Host (10:1 MOI)
E_BM_TK_V_naked <- beta_BM_TK_naked*(10)^4 #E calculated with Virus
