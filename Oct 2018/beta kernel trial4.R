...
#BROWNIAN MOTION
#BM_Beta_Burd <- ((2/3)*(K*Temp/mu)*((Reh+Rehv)^2/(Reh*Rehv)))*1e6 #in cm2/s  
...
D_TK_host <- ((K*Temp)/(6*pi*mu*Reh_calc))*(10)^4 #in cm2/s diffusity for a spherical host
...
Beta_BM_TK_calc <- ((4*pi)*(D_TK_host+D_TK_virus)*((Reh_calc+Rehv)*100)) #Brownian motion TK formula, in cm3/s
Beta_BM_TK_naked <- ((4*pi)*(D_TK_host+D_TK_virus)*((Reh_naked+Rehv)*100)) #Brownian motion TK formula, in cm3/s


#encounters are in encounters cm3/s then converted to encounters/ cm3 s
E_BM_TK_HV <- Beta_BM_TK*(10)^4*(10)^3 #E calculated with Virus and Host (10:1 MOI)
E_BM_TK_V <- Beta_BM_TK*(10)^4 #E calculated with Virus 
#E_BM_Burd_HV <- BM_Beta_Burd*(10)^4*(10)^3 #E calculated with Virus and Host (10:1 MOI)
#E_BM_Burd_V <- BM_Beta_Burd*(10)^4 #E calculated with Virus
...
Ehv_SinkVel <- ((2*((90E-9)^2)*(9.8*(86400)^2)*(Den_CH2O-Den_CH2O))/9*(mu*10))*864 #equals to 0

PIC$beta <- (pi*(Reh+Rehv)^2*(abs(PIC$SinkVel-Ehv_SinkVel))*(1e6/86400)) #in encounters cm3/s
