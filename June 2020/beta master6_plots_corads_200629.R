...
ggplot(data=PIC, aes(x=PICpercellpg, y=log10(beta_DS), color=group, shape=Strain)) + geom_point(size=5)+theme_Publication()+
...
liths <- data.frame (group=c("coccolith"), Den_celltotal =2.6, SinkVel = 0.45, beta_DS = 2.70*10^-6)

#edited as lab suggested
resize.win (6,7)
ggplot(data=PIC, aes(x=SinkVel, y=log10(beta_DS), color=group, shape=Strain)) + geom_point(size=5)+theme_Publication()+
  geom_point (data=liths, aes(x=SinkVel, y=log10(beta_DS), color="coccolith"), size=8, shape=20) +
...
ggplot(data=PIC_newdata, aes(x=SinkVel, y=log10(beta_DS), color=group)) + geom_point(size=5)+theme_Publication()+
  geom_point (data=liths, aes(x=SinkVel, y=log10(beta_DS), color="coccolith"), size=8, shape=20) +
  labs(y = expression(log[10]~beta[S]~("encounters "~mL~d^-1)), x = expression("sinking velocity "~("m"~d^-1)))  +
  theme(legend.direction = "horizontal", legend.box = "vertical", legend.title = element_blank()) +  
...
#turbulence
resize.win(6,6)
turb$group <- factor (turb$group,labels= c("naked", "calcified", "coccolith"))
...
all$group <- factor (all$group,labels= c("naked", "calcified", "coccolith"))
...
calmstormy$encounters <- calmstormy$beta_all*calmstormy$propEhV*calmstormy$prophostnum #total enc
...
calmstormy$group <- reorder.factor (calmstormy$group, new.order = c("naked", "calcified", "coccolith"))
...
allsteps_comb$perencounters <- (allsteps_comb$enccomb/allsteps_comb$prophostnum)
allsteps_comb$peradsorbed <- (allsteps_comb$adscomb/allsteps_comb$prophostnum)
allsteps_comb$perinf <- (allsteps_comb$infcomb/allsteps_comb$prophostnum)
...
##percentage of parameters
allsteps_comb.stormy$perencounters <- (allsteps_comb.stormy$enccomb/allsteps_comb.stormy$prophostnum)
allsteps_comb.stormy$peradsorbed <- (allsteps_comb.stormy$adscomb/allsteps_comb.stormy$prophostnum)
allsteps_comb.stormy$perinf <- (allsteps_comb.stormy$infcomb/allsteps_comb.stormy$prophostnum)
...
allsteps_comb.stormy$perencounters_noenc <- 1- allsteps_comb.stormy$perencounters
allsteps_comb.stormy$perads_noads <- 1- allsteps_comb.stormy$peradsorbed
allsteps_comb.stormy$perinf_noinf <- 1- allsteps_comb.stormy$perinf
...
#infected
ggplot(melt_inf.stormy %>% filter(!(group %in% c("Li"))), aes(x = "", y = value, fill=variable)) +
  geom_bar(width = 1, stat = "identity", color = "black") +
  coord_polar("y", start = 0)+
  scale_fill_manual(values=c("#800080", "#FFFFFF")) +
  theme_void() +  theme (legend.position = "bottom", legend.text = element_text(size = 15), 
...
##percentage of parameters
allsteps_comb.calm$perencounters <- (allsteps_comb.calm$enccomb/allsteps_comb.calm$prophostnum)
allsteps_comb.calm$peradsorbed <- (allsteps_comb.calm$adscomb/allsteps_comb.calm$prophostnum)
allsteps_comb.calm$perinf <- (allsteps_comb.calm$infcomb/allsteps_comb.calm$prophostnum)
...
allsteps_comb.calm$perencounters_noenc <- 1- allsteps_comb.calm$perencounters
allsteps_comb.calm$perads_noads <- 1- allsteps_comb.calm$peradsorbed
allsteps_comb.calm$perinf_noinf <- 1- allsteps_comb.calm$perinf
...
#infected
ggplot(melt_inf.calm %>% filter(!(group %in% c("Li"))), aes(x = "", y = value, fill=variable)) +
  geom_bar(width = 1, stat = "identity", color = "black") +
  coord_polar("y", start = 0)+
  scale_fill_manual(values=c("#800080", "#FFFFFF")) +
  theme_void() +  theme (legend.position = "bottom", legend.text = element_text(size = 15), 
...
allhost <- allhost %>%
  mutate (prophost = case_when (group=="Cc" ~ hostnum,
                                group=="Nc" ~ hostnum*0.1,
                                group=="Li" ~ hostnum*25))

allhost <- allhost %>%
  mutate (virnum = case_when (group=="Cc" ~ hostnum*30))
...
#calculate encounters fast slow
allhost$encounters <- allhost$beta_all*allhost$propEhV*allhost$prophostnum #total enc
...
ggplot(allhost_allsteps %>% filter (virus=="low"), aes(x=log10(prophostnum), y=log10(mid), color=group, shape=group))  + 
  geom_point (size=6) + 
  geom_errorbar(aes (ymax=log10(mid + stormy), ymin= log10(mid-calm)), width=0.5, size=1) +
  facet_grid(~parameter, labeller = labeller(parameter = as_labeller(variable_labs, label_parsed))) +
  theme_Publication2() + theme (legend.position = "bottom", legend.title =element_blank()) + 
  labs (y = expression(log[10]), x =expression (log[10]~"host concentration"~mL^-1)) + geom_hline(yintercept = log10(1), linetype="dashed") + scale_color_manual (values=c("#e41a1c", "#377eb8", "#4daf4a"))


###adsorption plot
library(readxl)
ads_plot <- read_excel("Postdoc-R/CSV Files/ads.xlsx")

ads_plot$group <- reorder.factor (ads_plot$group, new.order = c("Nc", "Cc", "Li"))

resize.win(4,4)
ggplot (ads_plot, aes(x=group, y=log10(adscoef), color=group)) + geom_boxplot() + theme_Publication() + 
  theme (axis.title.x = element_blank(), legend.position = "none") + labs (y= expression(log[10]~K[d]~("mL"~"day"^-1))) +
  scale_color_manual (values=c("#e41a1c", "#377eb8", "#4daf4a")) 

ggplot (ads_plot, aes(x=group, y=adsef*100, color=group)) + geom_boxplot() + theme_Publication() +  
