...
navice_alldata <- navice_alldata %>% filter (depth>=(-149) )

####------THESE ARE ALL THE UPPER 150M-----######
...
#edit in excel
write.table(melted_navice, "Postdoc-R/Exported Tables/melted_navice_150m.csv", sep=";", col.names=T, row.names=F)
...
ylim.sec2 <- c(0,100000)    # in this example, encounters
...
write.table(navice_EI, "Postdoc-R/Exported Tables/navice_EI_minus150m.csv", sep=";", col.names=T, row.names=F)
