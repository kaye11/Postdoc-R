...
probs <- as.data.frame(list (entitycode = as.factor(c("Nc", "Cc", "Li", "Nc", "Cc")), virus = c("slow","slow", NA, "fast", "fast"), adshst= c(1.14e-01, 4.82e-03, NA, 1.14e-01, 4.82e-03),  inf = c(0.3, 0.3, NA, 0.06, 0.06), ads = c(1, 1, 1.67e-02, 1, 1)))

probs$bioinf <- probs$adshst*probs$inf*probs$ads
...
melted_EI3$variable <- factor (melted_EI3$variable,levels= c("abundance", "encounters_propEhV",
                                                             "sucinf_prop"), 
                               labels = c("entity abundance", "encounters per day", "successful infection per day"))
resize.win(12,9)
