...
navice_alldata <- navice_alldata %>% filter (depth>=(-30) )
...
ylim.sec <- c(0,80000)    # in this example, encounters
...
probs <- as.data.frame(list (entitycode = as.factor(c("Nc", "Cc", "Li", "Nc", "Cc", "Li", "Nc", "Cc", "Li", "Nc", "Cc", "Li")), ads= c(0.05, 0.04, 0.36, 0.05, 0.04, 0.36, 0.05, 0.04, 0.36, 0.05, 0.04, 0.36), virus = c("slow","slow", NA, "fast", "fast", NA, "slow","slow", NA, "fast", "fast", NA), inf = c(0.3, 0.3, NA, 0.06, 00.6, NA, 0.3, 0.3, NA, 0.06, 0.06, NA), hst = c(0.25, 0.25, NA, 1, 1, NA, 1, 1, NA, 0.25, 0.25, NA)))
...
ylim.sec2 <- c(0,300)    # in this example, encounters
...
write.table(navice_EI, "Postdoc-R/Exported Tables/navice_EI_HV.csv", sep=";", col.names=T, row.names=F)
