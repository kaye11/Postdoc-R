...
#by date
ggplot(melted_navice %>% drop_na(encountersEhV), aes(x=as.factor(date2), y=log10(encountersEhV), color=Infection, shape=Infection)) +geom_boxplot() + geom_point (position = position_jitterdodge(), size=2) + labs (y = expression("Encounters "~day^-1), x= "date") + theme_Publication2() + theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.title = element_blank()) + facet_grid(~entitycode)

ggplot(melted_navice %>% drop_na(encounters_propEhV), aes(x=as.factor(date2), y=log10(encounters_propEhV), color=Infection, shape=Infection)) +geom_boxplot() + geom_point (position = position_jitterdodge(), size=2) + labs (y = expression("Encounters "~day^-1), x= "date") + theme_Publication2() + theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.title = element_blank()) + facet_grid(virus~entitycode)
...
##wind pdf
ggplot(navice_alldata, aes(x=log10(disrate), color=as.factor(depth))) + geom_density() +theme_Publication2() + labs (x="log10 dissipation rate") 

wind.sum <- summarySE(data=navice_alldata, measurevar = "disrate", group=c("depth"))
resize.win(9,6)
ggplot(wind.sum, aes(y=log10(disrate), x=depth)) + geom_point (size=4)  + geom_errorbar(aes(ymin=log10(disrate-se), ymax=log10(disrate+se))) + coord_flip() + scale_y_continuous(position = 'right') + theme_Publication2()

wind.sum2 <- summarySE(data=navice_alldata, measurevar = "disrate", group=c("depth", "Infection"))
ggplot(wind.sum2)  + geom_density(aes(x=log10(disrate), fill=Infection, color=Infection)) + facet_grid (Infection~.)+theme_Publication2() + labs (x="log10 dissipation rate") + theme (legend.position = "none")
