...
probs <- as.data.frame(list (entitycode = as.factor(c("Nc", "Cc", "Li", "Nc", "Cc", "Li")), virus = c("high","high", "high", "low", "low", "low"), ads= c(0.0376, 0.0130, 0.0226, 0.0376, 0.0130, 0.0226),  inf = c(0.3, 0.3, NA, 0.06, 0.06, NA)))
...
melted_EI3$variable <- factor (melted_EI3$variable,levels= c("abundance", "enccomb", "adscomb", "infcomb"), 
                               labels = c("entity abundance\n mL-1", "encounters\n day-1 mL-1", "entities with adsorbed\n EhVs day-1 mL-1", "lytic infections\n day-1 mL-1"))
...
resize.win(9.5,4)
ggplot(melted_EI3, aes(x=entitycode, y=log10(value), color=entitycode, shape=entitycode))  + geom_boxplot() + geom_point (position=position_jitterdodge()) + facet_grid(~variable) + theme_Publication() + theme (legend.title = element_blank(), axis.title.x = element_blank(), legend.position = "none") + labs (y="log10") + scale_color_manual (values=c("#e41a1c", "#377eb8", "#4daf4a")) +   geom_hline(yintercept = log10(1), linetype="dashed") 

ind.sum <- summarySE(data=navice_alldata, measurevar = "disrate", group=c("depth"))
resize.win(6,6)
