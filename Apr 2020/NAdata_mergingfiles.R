...
#for annotate
winddisrate <- read.csv("D:/Users/karengb/Exported Tables/winddisrate.csv")
NAdata <- read.csv("D:/Users/karengb/Exported Tables/NAdata_mergewithAli_rawer.csv")
...
myfun <- function(x,y) {
  ...
  x$max_epsilon=y$max_epsilon[apply(mat, 1, which.min)]
  x$disrate_1m=y$disrate_1[apply(mat, 1, which.min)]
...
write.table (NAdata_merge, "Exported Tables/NAdata_merge.csv", sep=",", col.names=T, row.names=F)
