...
ggplot(data = world)+ geom_sf(color="white", fill="white") + coord_sf(xlim = c(-70, 10), ylim = c(30, 75) , expand = FALSE)+ 
  ...
  scale_color_viridis(breaks = c(seq(-5, 9, 2))) + expand_limits(colour = c(seq(-5, 9, 2))) + theme_Publication2() + theme(legend.position = "right", legend.direction = "vertical", legend.key.height = unit (8, "line"), legend.key.width = unit (2, "line"),  legend.title=element_blank(), panel.spacing = unit(3, "lines"), axis.title = element_blank()) + facet_grid(watcon~step2) 
#no cores 10min
#12 cores 11 min
