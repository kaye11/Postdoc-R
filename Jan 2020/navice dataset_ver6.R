...
probs <- as.data.frame(list (entitycode = as.factor(c("Nc", "Cc", "Li", "Nc", "Cc", "Li")), virus = c("slow","slow", "slow", "fast", "fast", "fast"), ads= c(1.14e-01, 4.82e-03, 1.67e-02, 1.14e-01, 4.82e-03, 1.67e-02),  inf = c(0.3, 0.3, NA, 0.06, 0.06, NA)))
...
ggplot(navice_EI %>% drop_na(adstot_prop) , aes(x=as.factor(date2), y=log10(adstot_prop), color=Infection, shape=Infection)) +geom_boxplot() + geom_point (position = position_jitterdodge(), size=2) + labs (y = expression("adsorbed viruses "~day^-1), x= "date") + theme_Publication2() + theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.title = element_blank()) + facet_grid(virus~entitycode) +  scale_color_manual(values=c("#F8766D", "#B79F00", "#00BA38" ))
