...
library(readxl)
sixth <- read_excel("D:/Postdoc/Experiments/190211 Infection Sixth/summary.xlsx", 
...
sixth$treatment <- as.factor (sixth$treatment)
sixth$timef <- as.factor(sixth$time)
sixth$sytox <- sixth$sytox*100
...
sixth$countpermldiv <- sixth$countperml/10^6
sixth$ROSdiv <- sixth$ROS/10^4

sixth.long <- melt (data=sixth, id.vars=c("treatment", "time", "timef", "rep"), variable.name="stain")

sixth.long$group <- case_when(
  sixth.long$treatment =="sc"  ~ "still control",
  sixth.long$treatment =="si" ~ "still infected",
  sixth.long$treatment =="tc"  ~ "turbulent control",
  sixth.long$treatment =="ti" ~ "turbulent infected",
  TRUE ~ as.character(sixth.long$treatment)
)


require(tidyr)

sixth.long<- separate(sixth.long, group, into = paste("group", 1:2, sep = ""))
sixth.long$maingroup <- as.factor(paste(sixth.long$group1, sixth.long$group2, sep="-" ))

sixth.long$maingroup <- factor(sixth.long$maingroup,
                                  levels = c("still-control", "still-infected", 
                                             "turbulent-control", "turbulent-infected"),
                                  labels = c("still-control", "still-infected", 
                                             "turbulent-control", "turbulent-infected"))

sixth.long$group1 <- as.factor(sixth.long$group1)
sixth.long$group2 <- as.factor(sixth.long$group2)

#drop levels
#sixth.long.dropvp <- sixth.long[! sixth.long$group2=="viralparticles",  ]

#use all data
sum.all <- summarySE(sixth.long, measurevar = "value", groupvars = c("treatment", "timef", "time", "stain"))

scientific_10 <- scientific_10 <- function(x) {
  ifelse(x==0, "0", parse(text=gsub("[+]", "", gsub("e", " %*% 10^", scales::scientific_format()(x)))))
}

ggplotly(ggplot(data=sixth.long, aes(x=time, y=value, colour=treatment)) +geom_boxplot() + 
...
sum.all.group <- summarySE(sixth.long, measurevar = "value", 
...
sum.all.group$stain2 <- factor(sum.all.group$stain, levels=c("countperml", "sytox", "ROS"), 
                              labels =c ("Cell count", "%Sytox Stained", "RFU per cell (ROS)"))
...
ROS <- ggplot(data=sum.all.group %>% 
                      filter(stain %in% c("ROSdiv")), aes(x=time, y=value, colour=group2)) +
  geom_point(size=5) + 
  geom_errorbar(aes(ymin=value-se, ymax=value+se, width=5)) + 
  geom_smooth(method = 'loess', aes(colour=group2, fill=group2), alpha=0.2) + 
  labs (y= expression("RFU per cell (ROS)"~ scriptstyle (x) ~"10"^~4)) +
  scale_x_continuous(breaks=c(0, 24, 48, 72, 96, 120)) +
  theme_Publication() + 
  facet_grid(~group1) +
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(), legend.position ="none")
...
resize.win(12,16)
grid.newpage()
grid.draw(rbind(ggplotGrob(cellcount), ggplotGrob (ROS), ggplotGrob(sytox), size = "last"))
...
setwd("D:/R program")
require(openxlsx)
write.xlsx(sum.all.group, file = "Postdoc-R/Exported Tables/sixthExp_summary_sytoxROS.xlsx")
write.xlsx(sixth.long, file = "Postdoc-R/Exported Tables/sixthExp_sytoxROS.xlsx")
