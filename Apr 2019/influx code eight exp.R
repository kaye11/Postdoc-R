library(readxl)
influxdata <- read_excel("D:/Postdoc/Experiments/190401 Infection Eight/190418 run.xls")
View(influxdata)
...
influxdata$time <- case_when(
  influxdata$samplenumber <9  ~ "9d",
  influxdata$samplenumber >8 & influxdata$samplenumber <25 ~ "0",
  influxdata$samplenumber >24 & influxdata$samplenumber <41 ~ "24",
  influxdata$samplenumber >40 ~ "48", 
  TRUE ~ as.character(influxdata$samplenumber)
)

#change flowrate
flowrate <- (17.6+17.2+17.6+17)/4
...
influxdata <- read_excel("D:/Postdoc/Experiments/190401 Infection Eight/190422.xls")

#run source code
source("influxsource.R")

influxdata$time <- case_when(
  influxdata$samplenumber <17  ~ "120",
  influxdata$samplenumber >16 & influxdata$samplenumber <33 ~ "72",
  influxdata$samplenumber >32 & influxdata$samplenumber <49 ~ "96",
  influxdata$samplenumber >48 & influxdata$samplenumber <51 ~ "2d",
  influxdata$samplenumber >50 ~ "6d", 
  TRUE ~ as.character(influxdata$samplenumber)
)

#change flowrate
flowrate <- (21.6+20.4+21.2+20.8)/4
influxdata$count <- (influxdata$cellcount/(flowrate))*50*1000 #divide the flowrate by 2 if samplerun is 30 sec, do not divide by 2 if sample run is 60 sec
...
#9d of eight exp
ninedays <- influxdata %>% filter (time=="9d")
ninedays <- ninedays%>%
  arrange(factor(cell, c("EhV", "Bacteria")))

ninedays$treatment = c("still infected 1", "still infected 2", "still infected 3", "still infected 4",
                       "turbulent infected 1", "turbulent infected 2", "turbulent infected 3", "turbulent infected 4")

ninedays$countpermldiv <- ninedays$count/10^8

ninedays <- ninedays %>% separate(treatment, c("group1", "group2", "rep"))
ninedays$maingroup <- as.factor(paste(ninedays$group1, ninedays$group2, sep="-" ))

sum.9d <- summarySE(ninedays, measurevar = "countpermldiv", 
          groupvars = c("maingroup", "group1", "group2", "time", "cell"))

#2d and 6d of new lysate
newlys <- influxdata %>% filter (time %in% c("2d", "6d")) #stopped here
newlys$countpermldiv <- newlys$count/10^8

sum.newlys <- summarySE(newlys, measurevar = "countpermldiv", 
                    groupvars = c("time", "cell"))


#delete things
influxdata <- influxdata %>% filter (!(time %in% c("2d", "6d", "9d")))
...
influxdata$treatment <- rep_len (treatments, length.out = 192) #change according to sample number
...
setwd("D:/R program")
require(openxlsx)
write.xlsx(sum.all, file = "Postdoc-R/Exported Tables/EightExp_summary_virbac.xlsx")
write.xlsx(influxdata, file = "Postdoc-R/Exported Tables/EightExp_virbac.xlsx")
